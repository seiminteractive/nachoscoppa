// Países ISO 3166-1 alpha-2 con su nombre en español (generado en runtime con Intl.DisplayNames).
const CODES = (
...
function buildCountries() {
  let toName = (code) => code;
  try {
    const dn = new Intl.DisplayNames(["es"], { type: "region" });
    toName = (code) => dn.of(code) || code;
  } catch {
    /* Intl.DisplayNames no disponible: usar el código como nombre. */
  }
  return CODES.map((code) => ({ code, name: toName(code) }))
